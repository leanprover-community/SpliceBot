...
| `requires an argument, but the trigger workflow run predates argument support` | The trigger workflow ran from the PR's merge commit, which still pins a `splice.yaml` version older than argument support (review-comment workflows do not run from the default branch) | Update the PR branch (e.g. merge the latest base branch) so the merge commit picks up the caller repo's current workflow pin, then re-trigger |
